<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'add_blogs'){
		...
		$check_duplicate = "SELECT title FROM blogs WHERE title = '".$_REQUEST['title']."' ";
		...
		if($check_result == 'no'){
	    	...
	    	$sql_insert_blogs = "INSERT INTO blogs(title,url,description,image) VALUES ('".$title."','".$url."','".addslashes($description)."','".$blogs_main_image."')";
